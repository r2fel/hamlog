...
PRIVATE='^(CLAUDE\.md|КАК-ЗАПУСКАТЬ\.md|РАЗМЕЩЕНИЕ-В-ИНТЕРНЕТЕ\.md|Запустить QSO Log\.command|Открыть R2FEL-LOG \(приложение\)\.command|\.claude/)'
...
if gh release view "$TAG" --repo "$REPO" >/dev/null 2>&1; then
  ...
  gh release upload "$TAG" "$DMG" "$WIN" "$WIN78" --repo "$REPO" --clobber
else
