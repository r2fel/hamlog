...
// A couple of the same views, without the numbered marks, for the project's
// front page on GitHub: README.md shows what the program looks like, and
// nobody wants callout numbers there. Taken from the Russian run — that's the
// audience the page speaks to first.
const README_SHOTS = { contact: 'screenshot.webp', log: 'screenshot-log.webp' };

async function shot(name, crop, marks, margin = 10) {
  if (README_SHOTS[name] && lang === 'ru') {
    const clean = await js(`__rect(${JSON.stringify(crop)}, ${margin})`);
    const dir = path.join(PROJ, 'docs');
    fs.mkdirSync(dir, { recursive: true });
    await capture(clean, path.join(dir, README_SHOTS[name]), SHOT_SCALE);
    console.log(`  docs/${README_SHOTS[name]}`);
  }
